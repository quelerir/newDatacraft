...
import {
  purchasedModules,
  type PurchasedModuleKey,
} from '../data/settingsData';

export function SettingsPage() {
  const [activeModuleKey, setActiveModuleKey] =
    useState<PurchasedModuleKey>('stream');
  const [expandedSourceKeys, setExpandedSourceKeys] = useState<string[]>([
    'yandex-metrica',
  ]);
  const [selectedSettingsByModule, setSelectedSettingsByModule] = useState<
    Record<PurchasedModuleKey, string[]>
  >({
    stream: [],
    source: [],
    showcase: [],
  ...
  const activeModule = useMemo(
    () =>
      purchasedModules.find((module) => module.key === activeModuleKey) ??
      purchasedModules[0],
    [activeModuleKey],
  );
  ...
  const totalOptionsCount =
    activeModule.key === 'source'
      ? (activeModule.sourceItems ?? []).reduce(
          (sum, source) =>
            sum +
            source.settingsSections.reduce(
              (sectionSum, section) => sectionSum + section.options.length,
              0,
            ),
          0,
        )
      : activeModule.settingsSections.reduce(
          (sum, section) => sum + section.options.length,
          0,
        );
  const selectionSummaryLabel =
    activeModule.key === 'source'
      ? 'Включено настроек источников'
      : 'Выбрано опций';
      ...
      <main className="settings-page">
        <section className="settings-license-hero">
          <div className="settings-license-hero__copy">
            ...
            <p>
              У каждого модуля есть собственные настройки, а для одного и того
              же типа модуля можно хранить несколько профилей, например профил
              по умолчанию и тестовый профиль.
            </p>
          </div>
        </section>
          ...
          <div className="settings-license-grid">
            {purchasedModules.map((module) => {
              ...
              return (
                ...
                >
                  <div className="settings-license-card__top"></div>
            ...
            <div className="settings-license-section">
              <div className="settings-license-explainer">
                <div className="settings-license-settings-summary">
                  ...
                  <strong>{selectedSettings.length}</strong> из{' '}
                  <strong>{totalOptionsCount}</strong>
                </div>
                <div className="settings-license-setting-sections">
                  {activeModule.key === 'source' &&
                  activeModule.sourceItems?.length
                    ? activeModule.sourceItems.map((source) => {
                        const isExpanded = expandedSourceKeys.includes(
                          source.key,
                        );
                        ...
                        return (
                          <section
                            key={source.key}
                            className={`settings-license-setting-section settings-license-setting-section--source${isExpanded ? ' is-expanded' : ''}`}
                          >
                            <button
                            ...
                            >
                              ...
                              <span
                                className={`settings-license-source-accordion__chevron${isExpanded ? ' is-expanded' : ''}`}
                                aria-hidden="true"
                              >
                                +
                              </span>
                            ...
                            {isExpanded ? (
                              <div className="settings-license-source-accordion__body">
                                {source.settingsSections.map((section) => (
                                  <div
                                    key={`${source.key}-${section.title}`}
                                    className="settings-license-source-accordion__group"
                                  >
                                    <div className="settings-license-setting-section__title">
                                      {section.title}
                                    </div>
                                    <div className="settings-license-setting-list">
                                      {section.options.map((option) => {
                                        ...
                                        const isSelected =
                                          selectedSettings.includes(optionKey);
                                        ...
                                        return (
                                          ...
                                          >
                                            <div className="settings-license-setting-option__content">
                                              <span className="settings-license-setting-option__label">
                                                {section.title}
                                              </span>
                                              <strong>{option}</strong>
                                            </div>
                                            <span
                                              className={`settings-license-switch${isSelected ? ' is-on' : ''}`}
                                              aria-hidden="true"
                                            >
                    ...
                    : activeModule.settingsSections.map((section) => (
                        <section
                          key={section.title}
                          className="settings-license-setting-section"
                        >
                          <div className="settings-license-setting-section__title">
                            {section.title}
                          </div>
                          <div className="settings-license-setting-list">
                            {section.options.map((option) => (
                              <button
                                ...
                                type="button"
                                onClick={() =>
                                  toggleSetting(activeModule.key, option)
                                }
                              >
                                <div className="settings-license-setting-option__content">
                                  <span className="settings-license-setting-option__label">
                                ...
                                <span
                                  className={`settings-license-switch${selectedSettings.includes(option) ? ' is-on' : ''}`}
                                  aria-hidden="true"
                                >
                                  <span className="settings-license-switch__thumb" />
                                </span>
